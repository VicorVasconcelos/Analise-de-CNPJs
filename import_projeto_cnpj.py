...
Atualizado para usar os arquivos CSV reais da Receita Federal
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('import_projeto_cnpj.log'),
...
class CNPJProjectImporter:
    ...
    def find_csv_file(self, folder_name):
        ...
        if not folder_path.exists():
            logging.warning(f"⚠️ Pasta não encontrada: {folder_path}")
            return None
        
        csv_files = list(folder_path.glob("*.csv")) + list(folder_path.glob("*.CSV"))
        if not csv_files:
            logging.warning(f"⚠️ Nenhum arquivo CSV encontrado em: {folder_path}")
            return None
    ...
    def import_table(self, folder_name, config):
        ...
        try:
            ...
            if current_count > 0:
                logging.info(f"⏭️ Tabela {table_name} já possui {current_count} registros. Pulando...")
                return True
            ...
            logging.info(f"✅ {table_name}: {final_count:,} registros importados com sucesso!")
    ...
    def create_indexes(self):
        ...
        for index_sql in indexes:
            try:
                ...
                logging.warning(f"   ⚠️ Erro ao criar índice: {e}")
    ...
    def import_all_data(self):
        ...
        # Ordem de importação (tabelas de referência primeiro)
        import_order = [
            'Cnaes', 'Motivos', 'Municipios', 'Naturezas', 
            'Paises', 'Qualificacoes', 'Empresas0', 
            'Estabelecimentos0', 'Simples'
        ]
        ...
        for folder_name in import_order:
            if folder_name in self.folder_mapping:
                ...
                logging.info(f"\n📋 {success_count + 1}/{total_tables}: {config['table'].upper()}")
        ...
        logging.info(f"⏱️ Tempo total: {duration}")
...
def main():
    ...
    logging.info(f"🗄️ Banco de dados: {importer.db_path}")
